...
contract TinyLendTest is Test {
    ...
    function test_Withdraw() public {
        ...
        assertEq(tinyLend.suppliedUnderlying(alice, address(mockToken)), depositAmount - withdrawAmount, "remaining supplied amount should be deposit minus withdrawal");
        vm.stopPrank();
    }
    ...
    function test_BorrowAndRepay() public {
        ...
        assertEq(tinyLend.borrowedUnderlying(bob, address(mockToken)), borrowAmount, "borrowed amount should match requested amount");
        
        // repay the loan
        mockToken.approve(address(tinyLend), borrowAmount);
        tinyLend.repay(address(mockToken), borrowAmount);
        assertEq(tinyLend.borrowedUnderlying(bob, address(mockToken)), 0, "borrowed amount should be 0 after full repayment");
        vm.stopPrank();
    }
